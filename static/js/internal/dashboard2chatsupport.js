...
console.log($("div.leftmain").attr('data-profiles'));

var profiles_list = JSON.parse($("div.leftmain").attr('data-profiles'));
...
//      resetUI();
  ...
  // Get a reference to the database service
    ...
    var chatRef = firebase.database().ref('support/chat');
  ...
  // ...
    ...
    chatRef.on('child_added', function(data) {
				...
				data.forEach(function(child){
					var message= {customerProfileID:child.val().customerProfileID,
...
//				console.log("id");console.log(o.id);
				console.log("code1");console.log(code1);

				let obj = profiles_list.find(o => o.id == code1);
				...
				if(obj != null){
					derbii_title = "";
					if(obj.user__first_name != null){
						derbii_title += obj.user__first_name;
					}
					if(obj.user__last_name != null){
						derbii_title += " ";
						derbii_title += obj.user__last_name;
					}
/*					if(obj.simplev__phone__number != null){
						derbii_title1 += obj.simplev__phone__number;
					}
					if(obj.user__email != null){
						derbii_title1 += ", ";
						derbii_title1 += obj.user__email;
					}*/
					if(obj.id != null){
//						derbii_title1 += ", ";
						derbii_title1 += obj.id;
					}
				}

				if(obj != null){
					var indexObj = profiles_list.indexOf(obj);
					console.log("haha", div_lasttimes[code1]);
					obj['last_time'] = div_lasttimes[code1];
					profiles_list[indexObj] = obj;
				}

//				var innerHTML = '<div id="'+data.key+'" class="thread border-box clearfix shadow-text" data-customerProfileID="'+data.key+'" data-gh=\''+JSON.stringify(models)+'\'>';
				innerHTML = '<div class="code">';
				innerHTML += '<table class="stdtable"><tr><td valign="center" style="width:80%;">';
				innerHTML += derbii_title;
				innerHTML += '</td><td valign="center" style="width:20%;" class="rightalign"><span class="unread weight700 colorprim500span"></span></td></tr></table>';
				innerHTML += '</div>';
				innerHTML += '<div class="people">';
				innerHTML += '<table class="stdtable"><tr><td valign="center">';
				innerHTML += derbii_title1;
...
//				$(".leftmain > .second").append(innerHTML);
					...
					if($("#searchthreads").val().trim() == '' || $("#searchthreads").val() == null){
						...
						profiles_list.forEach(function(derbii){
							if (derbii.id in thread_boxes){
								$(".leftmain > .second").append(thread_boxes[derbii.id]);
							}
						});
					}

					firebase.database().ref('derbiisupportchat_admin/'+code1+'/last_read/'+$("div.leftmain").attr('data-user-id')).once('value').then(function(data){
						if (data.val() != null){
...
//					if(child.key != div_lastkeys[data.key] && divK[data.key].indexOf(child.key) != -1){
						...
						var newobject= {
	            			...
	            			senderfacebookID : child.val().senderFacebookID,
	            			sendergoogleID : child.val().senderGoogleID,
	            			customerProfileID : child.val().customerProfileID,
	            			time : child.val().time,
							notification:child.val().notification,
							type:child.val().type,
							people:child.val().people,
...
//						console.log(datanew);
						...
						let obj = profiles_list.find(o => o.id == data.key);
						if(obj != null){
							var indexObj = profiles_list.indexOf(obj);
						...
						if ($('.thread.selected').length && $('.thread.selected').attr('id') == data.key){
							...
							if(child.val().senderName != 'bot'){
								...
								firebase.database().ref('derbiisupportchat_admin/'+code1+'/last_read/'+$("div.leftmain").attr('data-user-id')).set({time:new_time});
							}
						}else{
							if(child.val().senderName != 'bot'){
...
//	    var state = e.originalEvent.state;
	    ...
	    if($(this).hasClass('notification')) {
	    	queueRef.push().set({
	    		_state: 'chat_support_notification_start',
            	customerProfileID : $('.thread.selected').attr('data-customerProfileID'),
...
//						$("#dashboardmainright").html('').html(data);
			...
			that=$(this);
				...
				if (true){
					...
					var code = $('.thread.selected').attr('data-customerProfileID');
					...
					let obj = profiles_list.find(o => o.code == code);
					if(obj != null){
						var indexObj = profiles_list.indexOf(obj);
					...
					var texts = div[$('.thread.selected').attr('data-customerProfileID')];
					var textHs = divH[$('.thread.selected').attr('data-customerProfileID')];
					var mKeys = divK[$('.thread.selected').attr('data-customerProfileID')];

					texts.push($(this).html());
					textHs.push('');
					mKeys.push(chatRefPush.key);
					div[$('.thread.selected').attr('data-customerProfileID')] = texts;
					divH[$('.thread.selected').attr('data-customerProfileID')] = textHs;
					divK[$('.thread.selected').attr('data-customerProfileID')] = mKeys;
...
//					console.log(datanew);
					...
					queueRef.push().set({
            			...
            			senderName : 'bot',
            			senderProfileID : 'bot',
            			customerProfileID : code,
            			messageType: "text",
            			_state: 'message_support_start'
					});
...
//		$("#sonicbox").html(sonic_blue_black.canvas);				
		...
		var code = $(this).attr('data-customerProfileID');
		var texts = div[code];
...
//		History.pushState(null, code, "?thread="+code);
		...
		var date = new Date();
		var time = date.getTime();
		firebase.database().ref('derbiisupportchat_admin/'+code+'/last_read/'+$("div.leftmain").attr('data-user-id')).set({time:time});
...
//			let obj1 = profiles_list.filter(o => o.id.toLowerCase().indexOf(val.toLowerCase()) != -1);
			let obj1 = profiles_list.filter(o => o.id == val.toLowerCase());
			let obj2 = profiles_list.filter(o => (o.user__first_name == null ? false : (o.user__first_name.toLowerCase().indexOf(val.toLowerCase()) != -1) ));
			let obj3 = profiles_list.filter(o => (o.user__last_name == null ? false : (o.user__last_name.toLowerCase().indexOf(val.toLowerCase()) != -1) ));
			//let obj4 = profiles_list.filter(o => (o.user__email == null ? false : (o.user__email.toLowerCase().indexOf(val.toLowerCase()) != -1) ));
//			let obj4 = profiles_list.filter(o => (o.simplev__phone__number == null ? false : (o.simplev__phone__number.toLowerCase().indexOf(val.toLowerCase()) != -1) ));
