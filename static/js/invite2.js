var request, pickupzone_selected = false, dropzone_selected=false, currentIndex = 0;
...
var verifyFields = function(index){
	switch(index){
		...
		case 0:
			...
			if ($.trim(pickupzone.val()) != '' && $.trim(dropzone.val()) != '' && pickupzone_selected && dropzone_selected){
				return true;
			}else{
				...
				request=$.post("https://derbii.com/ajax/hello/signup", { 
					...
					function(data) {
						if (data.status == "success"){
							$('.invite_main > div > div.left').stop().hide();
							$('.invite_main > div > div.right').stop().hide();
							$('.invite_main > div > div.postsignup').stop().show();
						}else{
							that1.text('Sign up');
...
$(document).ready(function(){
	...
	$('.invite_main > div > div.popup > div').stop().hide();
	...
	$(".timebox.hour").live('click', function(e){
		$('.invite_main > div > div.popupscreen').stop().show();
		$('.invite_main > div > div.popup').stop().show();
		$('.invite_main > div > div.popup > div.hour').stop().show();
	...
	$(".timebox.minute").live('click', function(e){
		$('.invite_main > div > div.popupscreen').stop().show();
		$('.invite_main > div > div.popup').stop().show();
		$('.invite_main > div > div.popup > div.minute').stop().show();
		timebox_selected = $(this);
	});
	...
	$(".timebox.tod").live('click', function(e){
		$('.invite_main > div > div.popupscreen').stop().show();
		$('.invite_main > div > div.popup').stop().show();
		$('.invite_main > div > div.popup > div.tod').stop().show();
		timebox_selected = $(this);
	});

	$(".popup .closebtn").live('click', function(e){
		$('.invite_main > div > div.popup > div').stop().hide();
		$('.invite_main > div > div.popupscreen').stop().hide();
		$('.invite_main > div > div.popup').stop().hide();
	...
	$(".genderbox").live('click', function(e){
  	...
  	$(".nextbtn").live('click', function(e){
  		var that = $(this);
  		var index = that.closest('.rightinner').index();
...
//		increaseArea: '20%' // optional
	...
	var zone_options = JSON.parse($(".invite_main > div > div.right").attr('data-zones'));
